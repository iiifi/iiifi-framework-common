...
public class HttpKitUtils {
	...
	private static final TrustAnyHostnameVerifier trustAnyHostnameVerifier = new HttpKitUtils().new TrustAnyHostnameVerifier();

	private static SSLSocketFactory initSSLSocketFactory() {
		try {
			TrustManager[] tm = { new HttpKitUtils().new TrustAnyTrustManager() };
	...
	public static String get(String url, Map<String, String> queryParas,
		...
		try {
			conn = getHttpConnection(buildUrlWithQueryString(url, queryParas),
					GET, headers);
			conn.connect();
			logger.debug("HttpKit get 请求地址：{}", conn.getURL());
			String result = readResponseString(conn);
			logger.debug("HttpKit get 响应结果：{}", result);
			return result;
		} catch (Exception e) {
			throw new RuntimeException(e);
		} finally {
	...
	public static String post(String url, Map<String, String> queryParas,
		...
		try {
			...
			logger.debug("HttpKit post 请求地址：{}", conn.getURL());
			OutputStream out = conn.getOutputStream();
			out.write(data.getBytes(CHARSET));
			out.flush();
			out.close();
			String result = readResponseString(conn);
			logger.debug("HttpKit post 请求返回参数：{}", result);
			return result;
		} catch (Exception e) {
			throw new RuntimeException(e);
		} finally {
